...
static int	fill_direction_textures(t_texinfo *textures, char *line, int j)
{
	...
	else if (line[j] == 'S' && line[j + 1] == 'O' && !textures->north)
		textures->south = get_texture_path(line, j + 2);
	else if (line[j] == 'W' && line[j + 1] == 'E' && !textures->north)
		textures->west = get_texture_path(line, j + 2);
	else if (line[j] == 'E' && line[j + 1] == 'A' && !textures->north)
		textures->east = get_texture_path(line, j + 2);
	else
		return (ERR);
}

static int	ignore_whitespace_get_info(t_data *data, char **map, int i, int j)
{
...
int	get_file_data(t_data *data, char **map)
{
	...
	while (map && map[i])
	{
		...
		while (map[i][j])
		{
			ret = ignore_whitespace_get_info(data, map, i, j);
			if (ret == BREAK)
				break ;
			else if (ret == FAILURE)
				return (FAILURE);
